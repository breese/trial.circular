...
> class span;
----
The span template class is a circular view of some contiguous memory. The memory
is not owned by the span. The owner must ensure that the span is destroyed before
...
If the span is not full, then elements are moved. Linear time complexity.
 ...
 +
 _Remarks:_ `noexcept` if `value_type` is nothrow _MoveConstructible_ and _MoveAssignable_.
| `constexpr{wj}footnote:constexpr11[] void normalize() noexcept(_see Remarks_)` | Rotates elements such that the span starts at the beginning of the storage. Normalization invalidates pointers and references, but does not invalidate iterators.
 +
 +
