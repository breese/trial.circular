...
subspans. These types fulfill the _RandomAccessRange_ requirements, which means
that they have `begin()` and `end()` functions returning a _RandomAccessIterator_.
...
| `segment` | _RandomAccessRange_ with `value_type`
| `const_segment` | _RandomAccessRange_ with `const value_type`
|===
