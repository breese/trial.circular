...
need are well-behaved iterators. The circular span iterators are bidirectional
iterators that allows you to traverse all elements from the front to the back,
or vice versa with reverse iterators. The iterators automatically handle that
the elements may wrap around the underlying storage.
...
subspans. These types fulfill the _BidirectionalRange_ requirements, which means
that they have `begin()` and `end()` functions returning a _BidirectionalIterator_.
Beyond that, the types are unspecified.

There are two member functions that returns a subspan: `first_segment()` returns
...
| `segment` | _BidirectionalRange_ with `value_type`
