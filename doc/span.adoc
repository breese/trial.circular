...
| `template <typename InputIterator>
 ...
 +
 _Remarks:_ `noexcept` if `value_type` is nothrow _CopyAssignable_.
| `constexpr{wj}footnote:constexpr11[] value_type move_front() noexcept(_see Remarks_)` | Removes and returns an element from the beginning of the span.
 +
 +
...
| `constexpr{wj}footnote:constexpr11[] value_type move_back() noexcept(_see Remarks_)` | Removes and returns an element from the end of the span.
 +
 +
 The removed element in the underlying storage is left in a moved-from state.
 +
 ...
 +
 _Remarks:_ `noexcept` if `value_type` is nothrow _MoveConstructible_.
| `constexpr{wj}footnote:constexpr11[] void increment_front() noexcept` | Inserts unspecified element at the beginning of the span.
 +
 +
 Makes room for one element at the front. The inserted element is in an unspecified but valid state.
 +
 +
 If the span is full, then the element is taken from the end of the span. This effectively rotates the span without touching the elements in the underlying storage. Otherwise, the span is enlarged.
 +
 +
 _Expects:_ `capacity() > 0`
| `constexpr{wj}footnote:constexpr11[] void increment_front(size_type count) noexcept` | Inserts unspecified elements at the beginning of the span.
 +
 +
...
| `constexpr{wj}footnote:constexpr11[] void increment_back() noexcept` | Inserts unspecified element at the end of the span.
 +
 +
 Makes room for one element at the back. The inserted element is in an unspecified but valid state.
 +
 +
 If the span is full, then the element is taken from the beginning of the span. This effectively rotates the span without touching the elements in the underlying storage. Otherwise, the span is enlarged.
 +
 +
 _Expects:_ `capacity() > 0`
| `constexpr{wj}footnote:constexpr11[] void increment_back(size_type count) noexcept` | Inserts unspecified elements at the end of the span.
 +
 +
 ...
 +
 _Expects:_ `count \<= capacity()`
| `constexpr{wj}footnote:constexpr11[] void decrement_front() noexcept` | Removes element from the beginning of the span.
 +
 +
 ...
 +
 _Expects:_ `size() > 0`
| `constexpr{wj}footnote:constexpr11[] void decrement_front(size_type count) noexcept` | Removes the specified number of elements from the beginning of the span.
 +
 +
...
| `constexpr{wj}footnote:constexpr11[] void decrement_back() noexcept` | Removes element from the end of the span.
 +
 +
 The removed element in the underlying storage is not destroyed.
 +
 +
 _Expects:_ `size() > 0`
| `constexpr{wj}footnote:constexpr11[] void decrement_back(size_type count) noexcept` | Removes the specified number of elements from the end of the span.
 +
 +
 The removed elements in the underlying storage are not destroyed.
 +
 ...
 +
 _Expects:_ `count \<= size()`
| `constexpr{wj}footnote:constexpr11[] void rotate_front() noexcept(_see Remarks_)` | Moves elements such that the span starts at the beginning of the storage.
 +
 +
