...
| `template <typename InputIterator>
 +
 constexpr{wj}footnote:constexpr11[] void push_back(value_type) noexcept(_see Remarks_)` | Inserts elements from iterator range at the end of the circular span.
 +
 +
 _Constraint:_ `value_type` must be _CopyAssignable_.
 +
